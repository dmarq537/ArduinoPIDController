...
// ===================== Adjust here =====================
/*** Setpoint & behavior ***/
float x_set_cm        = 25.0;     // distance target (cm)
const float STARTUP_DUTY = 0.52;  // open-loop duty for startup window (0..1)
const unsigned long STARTUP_MS = 0000UL;
...
void setup() {
    ...
    delay(1000);
    #if USE_PLOTTER
      Serial.println(F("Setpoint,Measured,Error,Duty"));  // Headers for plotter
    #else
...
void loop() {
  ...
  setDuty(dutyCmd);
    ...
    if (millis() - lastPlotMs >= PLOT_RATE_MS) {
      lastPlotMs = millis();
